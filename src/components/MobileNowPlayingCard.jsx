...
const MobileNowPlayingCard = ({
  ...
  if (!currentStation) {
    return (
      <div className="bg-black/95 backdrop-blur-xl border-t border-white/20 p-4 pb-safe">
  ...
  return (
    <div className="bg-black/95 backdrop-blur-xl border-t border-white/20 p-4 pb-safe max-h-[80vh] overflow-y-auto">
      {/* Header with collapse button - better visibility and larger hit area */}
      ...
      {/* Large Artwork - positioned above station info as requested */}
      <div className="flex justify-center mb-6">
        <img
          src={currentStation.logo || currentGame.logo}
          alt={currentStation.name}
          className="w-32 h-32 rounded-lg object-contain bg-black/30 border border-white/10 shadow-lg"
          draggable="false"
        />
      </div>

      {/* Station Info with LIVE tag below artwork */}
      <div className="text-center mb-6">
        <div className="flex items-center justify-center gap-2 mb-2">
          <span className="text-xs px-3 py-1 rounded-full bg-red-600 text-white font-bold tracking-wide animate-pulse">
            ● LIVE
          </span>
        </div>
        <div className="text-white font-bold text-xl mb-1">{currentStation.name}</div>
        <div className="text-sm text-gray-400 mb-3">{currentGame.name}</div>
        <div className="text-pink-300 text-lg font-medium">
      ...
      <div className="flex items-center justify-center gap-6 mb-6">
        <button
          ...
          className="p-3 rounded-full bg-white/10 hover:bg-white/20 border border-white/10 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
          aria-label="Previous"
        >
