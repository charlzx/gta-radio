    ...
    <>
      ...
      <header className="sticky top-0 z-20 bg-gradient-to-b from-black/60 to-black/30 backdrop-blur-xl border-b border-white/5 text-white">
        <div className="relative px-3 py-2">
          ...
          {/* Sidebar toggle (mobile, absolute right) - Remove on mobile per feedback */}
          {!isMobile && (
            <button
                ...
                (() => {
                  ...
                  return filteredStations.length > 0 ? (
                    <div className={`${isMobile ? 'space-y-3' : 'space-y-1'}`}>
                      ...
                      <div className={isMobile ? 'px-4 space-y-4' : ''}>
                        {filteredStations.map((station, index) => (
                          <div 
                            key={station.id}
                            className={isMobile ? 'min-h-[56px]' : ''}
                          >
                            <StationListItem
                              station={station}
                              index={index}
